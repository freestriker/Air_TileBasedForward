...
template<typename F, typename ...Args>
inline auto AirEngine::Core::IO::CoreObject::Thread::AddTask(F&& f, Args && ...args) -> std::future<typename std::invoke_result<F, AirEngine::Core::Graphic::Command::CommandBuffer*, Args ...>::type>
{
	using return_type = typename std::invoke_result<F, AirEngine::Core::Graphic::Command::CommandBuffer*, Args...>::type;

	auto task = std::make_shared<std::packaged_task<return_type(AirEngine::Core::Graphic::Command::CommandBuffer*)> >(
	...
	std::future<return_type> res = task->get_future();
	{
		...
		tasks.emplace([task](AirEngine::Core::Graphic::Command::CommandBuffer* const transferCommandBuffer) { (*task)(transferCommandBuffer); });
	}
