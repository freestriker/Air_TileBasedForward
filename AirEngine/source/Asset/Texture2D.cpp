...
void AirEngine::Asset::Texture2D::OnLoad(Core::Graphic::Command::CommandBuffer* transferCommandBuffer)
{
	...
	textureStagingBuffer.WriteData(_byteData.data(), textureStagingBuffer.Size());

	_textureInfoBuffer = new Core::Graphic::Instance::Buffer(sizeof(_textureInfo), VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
	Core::Graphic::Instance::Buffer infoStagingBuffer = Core::Graphic::Instance::Buffer(sizeof(_textureInfo), VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
	infoStagingBuffer.WriteData(&_textureInfo, infoStagingBuffer.Size());
...
AirEngine::Asset::Texture2D::Texture2D(Core::Graphic::Command::CommandBuffer* transferCommandBuffer, VkExtent2D extent, Texture2DSettings settings, std::vector<unsigned char>& byteData)
	: AssetBase(false)
	, _extent(extent)
	, _textureInfoBuffer(nullptr)
	, _image(nullptr)
...
std::vector<unsigned char>* AirEngine::Asset::Texture2D::ByteData()
{
	return &_byteData;
}
