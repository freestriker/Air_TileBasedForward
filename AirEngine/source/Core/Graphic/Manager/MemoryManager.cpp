...

{
	...
	Utils::Log::Exception("Failed to allocate memory chunk.", vkAllocateMemory(Graphic::CoreObject::VulkanInstance::VulkanDevice_(), &allocInfo, nullptr, &memory));
}

AirEngine::Core::Graphic::Manager::MemoryManager::MemoryChunk::~MemoryChunk()
{
	vkFreeMemory(Graphic::CoreObject::VulkanInstance::VulkanDevice_(), memory, nullptr);
}
	...
	: _defaultSize(defaultSize)
{
	...
	vkGetPhysicalDeviceMemoryProperties(Graphic::CoreObject::VulkanInstance::VulkanPhysicalDevice_(), &memProperties);
	...
	if (newSize > _defaultSize)
	{
		for (auto& pool : _pools)
		{
			if ((requirement.memoryTypeBits & (1 << pool->memoryTypeIndex)) && (_pools[pool->memoryTypeIndex]->properties & properties) == properties)
			{
				...
				Utils::Log::Exception("Failed to allocate exculsive memory.", vkAllocateMemory(Graphic::CoreObject::VulkanInstance::VulkanDevice_(), &allocInfo, nullptr, &newMemory));
...
void AirEngine::Core::Graphic::Manager::MemoryManager::ReleaseMemory(Instance::Memory& memory)
{
	...
	{
		vkFreeMemory(Graphic::CoreObject::VulkanInstance::VulkanDevice_(), memory._vkMemory, nullptr);
